...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    // emails are sent via backend function
                ...
                {sent ? (
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
                      {lang === 'el'
                        ? 'Θα επικοινωνήσουμε εντός μίας εργάσιμης ημέρας.'
                  ...
                  >
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <FormField label={c.form.name} error={errors.name} touched={touched.name}>
                        <input type="text" value={form.name}
                          ...
                          className={inputClass(touched.name, errors.name, touched.name)}
                          autoComplete="name" />
                      </FormField>
                      ...
                      <FormField label={c.form.email} error={errors.email} touched={touched.email}>
                        <input type="email" value={form.email}
                          ...
                          className={inputClass(touched.email, errors.email, touched.email)}
                          autoComplete="email" />
                      </FormField>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <FormField label={c.form.service} error={errors.service} touched={touched.service}>
                        <CustomSelect value={form.service}
                          onChange={v => { setField('service')(v); touch('service'); }}
                          options={c.services} placeholder={c.form.servicePh}
                          touched={touched.service} error={errors.service} />
                      </FormField>

                      <FormField label={c.form.budget} error={errors.budget} touched={touched.budget}>
                        <CustomSelect value={form.budget}
                          onChange={v => { setField('budget')(v); touch('budget'); }}
                          options={c.budgets} placeholder={c.form.budgetPh}
                          touched={touched.budget} error={errors.budget} />
                      </FormField>
                    </div>

                    <FormField label={c.form.details} error={errors.details} touched={touched.details}>
                      <textarea rows={5} value={form.details}
                        onChange={setField('details')} onBlur={() => touch('details')}
                    ...
                    <AnimatePresence>
                      {submitError && (
                        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className="flex items-center gap-2.5 text-sm text-amber-400/90 bg-amber-400/[0.07] border border-amber-400/20 rounded-xl px-4 py-3"
                          role="alert">
